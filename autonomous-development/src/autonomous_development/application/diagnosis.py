...
The response must be exactly one JSON object with these ten keys and no others:
observed_problem, affected_journey, confidence, competing_hypotheses,
likely_root_cause, proposed_change_class, expected_outcome, risks,
requested_paths, required_validation.
Use the exact key names above. Do not use alternate wrappers or fields such as
problem, diagnosis, action, classification, or root_cause. `requested_paths`
must contain at least one path and every requested path must remain inside the
allowed paths. Do not include Markdown fences or explanatory prose.
`confidence` must be a JSON number between 0 and 1, such as 0.85, never a
label or string such as "high" or "medium". `competing_hypotheses`, `risks`,
`requested_paths`, and `required_validation` must each be JSON arrays of strings.

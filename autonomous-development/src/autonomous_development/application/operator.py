...
from autonomous_development.ports.world_runtime import DevelopmentRuntimePort
...
class OperatorService:
    def __init__(
        ...
        *,
        ...
        runtime: DevelopmentRuntimePort | None = None,
    ) -> None:
    ...
    def status(self, request_id: str) -> OperatorStatus:
        ...
        return OperatorStatus(
            ...
            analysis=self._repository.get_analysis(request.id),
        )
    ...
    def prepare(self, request_id: str, *, operation_id: str) -> RequirementPreparation:
        ...
        try:
            self._requirements.revalidate(analysis)
        except Exception as exc:
            return self._needs_human(
                analyzing,
                kind="personal-context-revalidation",
                question=(
                    "The Personal World context used for this requirement changed before "
                    "autonomous implementation. Revalidate or correct personal context, then retry "
                    f"({type(exc).__name__})."
                ),
                choices=("revalidate-personal-context", "cancel"),
            )
        ...
        self._emit(
            ...
            payload={
                ...
                "personal_context_basis_refs": list(analysis.personal_context_basis_refs),
                "personal_context_revalidation_refs": list(

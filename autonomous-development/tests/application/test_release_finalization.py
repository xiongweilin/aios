...
class FakeRuntimeReporter:
    ...
    def ensure_assignment(self, **kwargs) -> None:
        raise AssertionError("release finalization must not create Runtime assignment")

    def complete_promoted_release(self, *, cycle, candidate, artifact, deployment, release) -> None:
        assert candidate.id == cycle.candidate_id
        assert artifact.id == cycle.artifact_id
        assert deployment.id == cycle.candidate_deployment_id
        self.completed.append(release.id)
...
def test_promoted_release_reports_domain_outcome_to_runtime() -> None:
    repository = MemoryReleaseRepository()
    runtime = FakeRuntimeReporter()
    service = ReleaseFinalizationService(
        ReleaseCatalogService(repository),
        runtime=runtime,
    )
    ...
    service.finalize(
        ...
        release_id="release-1",
        promoted_at=datetime.now(UTC),
        operation_id="finalize-runtime",
    )

    assert runtime.completed == ["release-1"]

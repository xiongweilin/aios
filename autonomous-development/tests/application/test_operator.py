...
class Runtime:
    """Records the generic Runtime boundary calls made while preparing a requirement."""

    def __init__(self) -> None:
        self.assignments: list[dict[str, object]] = []

    def ensure_assignment(self, **kwargs: object) -> None:
        self.assignments.append(kwargs)

    def complete_promoted_release(self, **kwargs: object) -> None:
        raise AssertionError("requirement preparation must not finalize a release")
...
class FailingRequirements:
    ...
    def revalidate(self, analysis: RequirementAnalysis) -> None:
        raise AssertionError("failed analysis must never reach revalidation")
...
def test_requirement_preparation_registers_the_runtime_responsibility(tmp_path: Path) -> None:
    runtime = Runtime()
    operator, _, _ = service(tmp_path, analysis(), runtime=runtime)

    prepared = operator.prepare("request-1", operation_id="prepare-runtime-1")

    assert prepared.status == "ready"
    assert len(runtime.assignments) == 1, "the Runtime must know the cycle before it starts"
    assignment = runtime.assignments[0]
    assert assignment["cycle"].id == prepared.cycle_id  # type: ignore[union-attr]
    assert assignment["target"].id == "target-1"  # type: ignore[union-attr]
    assert assignment["objective"].id == "objective-1"  # type: ignore[union-attr]
    assert assignment["baseline"].id == "release-1"  # type: ignore[union-attr]
    assert assignment["evidence_window"] is None, "a requirement carries no product evidence window"
...
def test_changed_personal_context_becomes_human_intervention(tmp_path: Path) -> None:
    operator, repository, _ = service(tmp_path, analysis())
    operator._requirements = Requirements(
        analysis(),
        revalidation_error=RuntimeError("Personal World basis requires revalidation"),
    )  # type: ignore[assignment]

    prepared = operator.prepare("request-1", operation_id="prepare-context-drift")

    assert prepared.status == "needs-human"
    intervention = repository.get_intervention("intervention:request-1")
    assert intervention is not None
    assert intervention.kind == "personal-context-revalidation"
    assert operator.get_request("request-1").cycle_id is None

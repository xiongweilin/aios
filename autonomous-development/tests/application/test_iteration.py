...
class FakeRuntimeReporter:
    ...
    def ensure_assignment(self, *, cycle, target, objective, baseline, evidence_window) -> None:
        del target, objective, baseline, evidence_window
        self.assignments.append(cycle.id)

    def complete_promoted_release(self, **kwargs) -> None:
        raise AssertionError("iteration preparation must not complete Runtime responsibility")
...
def test_feedback_to_change_proposal_is_replay_safe_and_scope_bounded(tmp_path: Path) -> None:
    ...
    assert runtime.assignments == ["cycle-1", "cycle-1"]

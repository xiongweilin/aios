...
On Windows, a host Syft/Grype invocation may fail with Docker's `The parameter is incorrect`
layer-cache compatibility error. The scanner then falls back only for that identified
compatibility signature to the pinned Syft and Grype container images, scanning a Docker archive
of the exact candidate image. Other scanner failures remain hard failures. The emitted SBOM and
vulnerability evidence records the transport (`host` or `docker-container`) and scanner image
identity when the fallback is used.
...
Run-specific acceptance output is intentionally not committed to the source tree. CI retains
acceptance evidence as workflow artifacts; local runs retain it in an operator-controlled evidence
location. The committed `acceptance/target/` definition and this runbook remain the source
contract for reproducing a fresh acceptance run.

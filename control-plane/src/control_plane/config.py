...
class ControlPlaneConfig:
    ...
    Universal responsibility, governance and durable semantic authority are owned
    by external World Runtime. This config owns only control-plane domain state.
    """
    ...
    state_db: Path = PROJECT_ROOT / "data" / "control-plane-domain.db"
    world_runtime_base_url: str = "http://127.0.0.1:18084"
    ...
    def load(cls, path: Path | None = None) -> ControlPlaneConfig:
        ...
        return cls(
            ...
            state_db=Path(str(runtime.get("state_db", base.state_db))),
            world_runtime_base_url=str(runtime.get("base_url", base.world_runtime_base_url)),
            world_runtime_timeout_seconds=float(
                runtime.get(
                    "timeout_seconds",
                    base.world_runtime_timeout_seconds,
                )
            ),
            ...
            artifact_root=Path(str(runtime.get("artifact_root", base.artifact_root))),

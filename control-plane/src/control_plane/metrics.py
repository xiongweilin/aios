...
Control-plane now owns only domain-local operational state, so this module is a
read-only projection over the local Domain Controller journal. It keeps the existing
dashboard metric names stable while making their source and compatibility semantics explicit.
"""
...
class ControlPlaneMetricsCollector:
    """Collect current profile metrics from control-plane domain projections."""
    ...
    def _works(self) -> tuple[list[Any], int]:
        try:
            return list(self.bridge.list_work()), 0
        except Exception:
            return [], 1
    ...
    def _today_calls(self) -> tuple[int, int]:
        try:
            events = self.bridge.journal.events()
        except Exception:
            return 0, 1
        today = datetime.now(UTC).date()
        count = 0
        for event in events:
            if not event.kind.endswith(_CALL_EVENT_SUFFIX):
                continue

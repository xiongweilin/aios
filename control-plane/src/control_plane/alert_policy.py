...
from .domain_store import DomainEvent
from .epistemic_profile import (
    build_repair_epistemic_profile,
    render_meta_control_directive,
)
from .provider_protocol import CapabilityResult, EffectClass, new_id
from .runtime_bridge import PersonalRuntimeBridge

_RESULT_EVENT = "control-plane.controller.capability-result"
_DECISION_EVENT = "control-plane.controller.decision"
...
def _ordered_decisions(
    ...
    controller_id: str,
) -> list[tuple[DomainEvent, ControllerDecision]]:
    values: list[tuple[DomainEvent, ControllerDecision]] = []
    for event in controller.ledger.events(stream=f"controller:{controller_id}"):
        if event.kind != _DECISION_EVENT:
            continue
        raw = event.payload.get("decision")
        if isinstance(raw, dict):
            values.append((event, ControllerDecision.model_validate(raw)))
    return sorted(values, key=lambda item: item[0].recorded_at)
...
def _event_result(event: DomainEvent) -> dict[str, Any]:
    raw = event.payload.get("result")
    return dict(raw) if isinstance(raw, dict) else {}


def _run_ref(events: list[DomainEvent]) -> str | None:
...
def _latest_run(bridge: PersonalRuntimeBridge, work_id: str) -> Any | None:
    runs = bridge.list_runs(work_id)
    return runs[-1] if runs else None


def _recent_reality_context(bridge: PersonalRuntimeBridge, controller_id: str) -> str:
...
@dataclass(frozen=True, slots=True)
class AutonomousRepairPolicy(StagedDomainPolicy):
    """Personal incident policy owned by the control-plane domain.

    Diagnosis, bounded execution and verification stay inside this Domain
    Controller. Universal Responsibility/Assignment state is reported through
    PersonalRuntimeBridge and is never reimplemented here.
    """

    controller: PersonalController = field(repr=False, compare=False)
    bridge: PersonalRuntimeBridge = field(repr=False, compare=False)
    prompt: str
    diagnosis_model: str
    execution_model: str
    repo: str | None = None
    ...
    def _session_cutoff(self, controller_id: str) -> datetime | None:
        ...
        return event.recorded_at if event is not None else None
    ...
    def _diagnosis_count(self, state: ControllerState) -> int:
        ...
        for event, decision in _ordered_decisions(self.controller, state.id):
            if cutoff is not None and event.recorded_at <= cutoff:
                continue
            if (
    ...
    def _diagnosis(self, state: ControllerState, *, retry_context: str = "") -> ControllerDecision:
        ...
        meta_directive = render_meta_control_directive(epistemic_profile)
        instruction = (
        ...
        if self._is_line_ending_cleanup():
            ...
            instruction += (
                "\n\nFor this synchronization alert, choose exactly one Runtime-qualified action. "
        ...
        parameters: dict[str, Any] = {
            ...
            "meta_intent": epistemic_profile.intent.kind.value,
            "meta_candidate_ref": epistemic_profile.intent.candidate_ref or "",
        }
        if self.repo:
    ...
    def _current_revision(self, state: ControllerState) -> RepairRevision | None:
    ...
    def _human_reopen_revision(self, state: ControllerState) -> ControllerDecision:
        ...
        return ControllerDecision(
            ...
            kind=ControllerDecisionKind.ASSESS_REVISION,
            revision=revision,
            reason="explicit owner direction reopens cognition through RepairRevision",
        )
    ...
    async def execute_work(self, state: ControllerState) -> None:
        ...
        run = _latest_run(self.bridge, work.id)
        if run is None:
            run = self.bridge.start_run(
                work.id,
        ...
        async def invoke(
            ...
            **parameters: Any,
            ...
            try:
                return await self.bridge.invoke_capability(
                    work.id,
                    capability,
                    run_id=run.id,
                    instruction=instruction,
        ...
        if safety == "unknown":
            ...
            execution_prefix = (
                "Prepare the exact line-ending-noise cleanup for the control-plane provider. "
                "Re-read the diagnosis evidence; do not run Git or shell commands. "
                "Do not edit files directly, and let the provider enforce the configured "
                "repository and semantic "
                "equality guard. Finish with a concise execution summary.\n\n"
            )
        elif sync_plan is not None:
            execution_prefix = (
                "Prepare the selected synchronization operation for the control-plane provider. "
                "Re-read the diagnosis evidence; do not run git or chezmoi commands. "
                "Do not modify files or access remote credentials. The provider will perform "
                "the exact bounded "
            ...
            execution_prefix = (
                ...
                "remote/deployment effects belong to Agent control-plane providers. Finish with a "
...
@dataclass(frozen=True, slots=True)
class ManualTaskPolicy(StagedDomainPolicy):
    ...
    def _revision(self, state: ControllerState) -> ControllerDecision:
        ...
        return ControllerDecision(
            ...
            kind=ControllerDecisionKind.ASSESS_REVISION,
            revision=revision,
            reason="return the materialized personal Work result through RepairRevision",
        )
    ...
    async def execute_work(self, state: ControllerState) -> None:
        ...
        observed = await self.bridge.invoke_capability(
            work.id,
            capability,
            instruction=instruction,
            run_id=run.id,
            resource_ref=self.repo if self.repo else None,
            parameters=parameters,
        )
        self.bridge.record_capability_result(
            controller_id=state.id,
...
async def drive_policy(
    ...
    *,
    ...
    """Drive one control-plane domain policy through its bounded local loop."""
    ...
    for _ in range(max_steps):
        if state.status is ControllerStatus.CLOSED:
            policy.bridge.report_completion(state)
            return state

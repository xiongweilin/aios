...
param(
    ...
    [switch]$EnsureWorldRuntimeOnly,
    [string]$WorldRuntimeDir = 'D:\agent\world-runtime',
...
function Ensure-WorldRuntime {
    param(
        [int]$Attempts = 12,
        [int]$DelaySeconds = 5
    )

    # 2026-09-21: control-plane 是 external Domain Controller，World Runtime
    # HTTP 服务（127.0.0.1:18086）是它的硬依赖。这里由 control-plane 监督链路
    # 自身拉起该依赖，避免再增加独立的 WorldRuntime 计划任务。
    for ($attempt = 1; $attempt -le $Attempts; $attempt++) {
        try {
            $probe = Invoke-WebRequest -Uri $WorldRuntimeHealthUrl -Method Get -TimeoutSec 3 -SkipHttpErrorCheck
            if ($probe.StatusCode -eq 200) {
                return $true
            }
        }
        catch {
            # Not healthy yet; start it below on the first attempt.
        }

        if ($attempt -eq 1) {
            $runtimePython = Join-Path $WorldRuntimeDir '.venv\Scripts\python.exe'
            $runtimeStateDb = Join-Path $ProjectDir 'data\world-runtime.db'
            if ((Test-Path -LiteralPath $runtimePython -PathType Leaf) -and (Test-Path -LiteralPath $runtimeStateDb -PathType Leaf)) {
                New-Item -ItemType Directory -Path $logDir -Force | Out-Null
                $env:PYTHONDONTWRITEBYTECODE = '1'
                $runtimeCode = "import uvicorn; from world_runtime.ledger import SQLiteLedger; from world_runtime.runtime import WorldRuntime; from world_runtime.service import create_app; ledger = SQLiteLedger(r'$runtimeStateDb'); app = create_app(WorldRuntime(ledger, runtime_id='world-runtime')); uvicorn.run(app, host='127.0.0.1', port=18086, log_level='warning')"
                # Start-Process joins ArgumentList elements with spaces without
                # quoting; the inline program must stay one argument for python -c.
                $quotedRuntimeCode = '"' + $runtimeCode + '"'
                Start-Process -FilePath $runtimePython -ArgumentList @('-c', $quotedRuntimeCode) -WindowStyle Hidden -RedirectStandardOutput (Join-Path $logDir 'world-runtime.stdout.log') -RedirectStandardError (Join-Path $logDir 'world-runtime.stderr.log')
            }
        }
...
if ($EnsureWorldRuntimeOnly) {
    if (Ensure-WorldRuntime) { exit 0 } else { exit 1 }
}

# Ensure the runtime dependency before deciding whether control-plane must start.
$null = Ensure-WorldRuntime

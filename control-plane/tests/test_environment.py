...
def test_environment_evaluation_covers_standing_control_plane_responsibilities(
    ...
    snapshot = evaluate_environment(
        ...
        {
            "recovery_ok": False,
            "recovery_missing_paths": [str(tmp_path / "backup")],
            "synchronization_ok": False,
            "synchronization_failures": [str(tmp_path / "project")],
            "synchronization_checked": 2,
            "known_garbage_count": 1,
            "known_garbage_paths": [str(tmp_path / "worktrees")],
        },
...
def test_expected_one_shot_containers_do_not_become_unexpected_exited_alerts(
    ...
    snapshot = evaluate_environment(
        make_config(tmp_path),
        {
            ...
            "docker_exited_container_names": ["sample-migration-1"],
            "docker_exited_container_records": [
                {"name": "sample-migration-1", "exit_code": 0}
            ],

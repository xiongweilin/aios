...
class FakeService:
    def __init__(self) -> None:
        ...
        self.analysis = RequirementAnalysis(
            ...
            acceptance_criteria=("bounded behavior is preserved",),
            requested_paths=("src/app.py",),
            expected_behavior=("the requested behavior is present",),
            risks=(),
            missing_information=(),
            ambiguity=(),
            validation_expectations=("run tests",),
            created_at=datetime(2026, 9, 19, tzinfo=UTC),
            personal_context_projection_ref="projection-1",
            personal_context_basis_refs=("personal-world:record-1@3",),
...
def test_operator_router_covers_submit_status_start_cancel_intervention_and_outbox() -> None:
    ...
    assert status.json()["commandVersion"].startswith("development-request-sha256:")
    assert status.json()["commandBindings"][0]["commandName"] == "requirement.cancel"
    assert status.json()["commandBindings"][0]["targetRef"] == "development:requirement:request-1"
    assert status.json()["intervention"]["interventionId"] == "intervention-1"
    assert status.json()["contextProjectionRef"] == "projection-1"
    assert status.json()["contextBasisRefs"] == ["personal-world:record-1@3"]
    assert status.json()["contextRevalidationRefs"] == []
...
def test_cancel_if_current_requires_authoritative_command_version() -> None:
    http, service = client()
    status_path = "/v1/operator/requirements/request-1"
    current = http.get(
        status_path,
        headers=signed("GET", status_path, "request-1"),
    )
    assert current.status_code == 200
    command_version = current.json()["commandVersion"]

    cancel_path = "/v1/operator/requirements/request-1/cancel-if-current"
    stale = http.post(
        cancel_path,
        headers={
            **signed("POST", cancel_path, "request-1"),
            "X-Expected-Command-Version": "development-request-sha256:stale",
        },
    )
    assert stale.status_code == 409
    assert service.value.status is DevelopmentRequestStatus.RECEIVED

    accepted = http.post(
        cancel_path,
        headers={
            **signed("POST", cancel_path, "request-1"),
            "X-Expected-Command-Version": command_version,
        },
    )
    assert accepted.status_code == 200
    assert accepted.json()["status"] == DevelopmentRequestStatus.CANCELLED.value
    assert accepted.json()["commandVersion"] != command_version
    assert accepted.json()["commandBindings"] == []
    assert service.value.status is DevelopmentRequestStatus.CANCELLED

...
def test_readiness_world_runtime_cutover_requires_contract_surface(
    tmp_path: Path,
    monkeypatch,
) -> None:
    engine, targets, releases = runtime_state(tmp_path)
    ...
    settings = configured(tmp_path)
    settings.world_runtime_mode = "cutover"
    ...
    assert not report.ready
    runtime_check = next(check for check in report.checks if check.name == "world-runtime")
    assert runtime_check.detail.startswith("contract mismatch:")

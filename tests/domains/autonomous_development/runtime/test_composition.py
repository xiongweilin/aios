...
class FakePersonalWorldProjectionClient:
    instances: ClassVar[list[FakePersonalWorldProjectionClient]] = []
    ...
    def __init__(
        ...
        *,
        ...
        workload_secret: str | None,
    ) -> None:
        self.base_url = base_url
        self.timeout_seconds = timeout_seconds
        self.bearer_token = bearer_token
        self.workload_secret = workload_secret
        self.ensured = False
        self.closed = False
...
def test_compose_runtime_wires_single_registered_target(
    ...
    monkeypatch.setattr(
        ...
        FakePersonalWorldProjectionClient,
    )
    monkeypatch.setattr(
        composition,
    ...
    FakePersonalWorldProjectionClient.instances.clear()
    runtime = compose_runtime(
        settings(
            tmp_path,
            database_url,
            personal_world_mode="required",
            personal_world_subject_id=UUID("11111111-1111-4111-8111-111111111111"),
            personal_world_bearer_token=SecretStr("pw-bearer"),
            personal_world_workload_hmac_secret=SecretStr("pw-workload"),
        )
    )
    ...
    assert len(FakePersonalWorldProjectionClient.instances) == 1
    personal_world = FakePersonalWorldProjectionClient.instances[0]
    assert personal_world.ensured
    assert personal_world.bearer_token == "pw-bearer"
    assert personal_world.workload_secret == "pw-workload"

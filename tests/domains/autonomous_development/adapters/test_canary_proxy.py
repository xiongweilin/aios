...
@pytest.mark.asyncio
async def test_proxy_uses_mount_relative_path_for_upstream_requests() -> None:
    ...
    product = create_canary_proxy(
        FakeRouteReader(route(100)),
        CanaryMetricsRegistry(),
        upstream_transport=httpx.MockTransport(upstream),
    )
    host = FastAPI()
    host.mount("/product", product)
    async with httpx.AsyncClient(
        transport=httpx.ASGITransport(app=host),
        base_url="http://proxy.local",
    ) as client:
        response = await client.get("/product/answer", params={"value": "hello"})

    assert response.status_code == 200
    assert observed_paths == ["/answer"]

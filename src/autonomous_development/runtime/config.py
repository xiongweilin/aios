...
class RuntimeSettings(BaseSettings):
    ...
    world_runtime_timeout_seconds: float = Field(default=3.0, gt=0)
    world_runtime_bearer_token: SecretStr | None = None
    personal_world_mode: Literal["disabled", "required"] = "disabled"
    personal_world_base_url: str = "http://127.0.0.1:8080"
    personal_world_timeout_seconds: float = Field(default=3.0, gt=0)
    personal_world_subject_id: UUID | None = None
    personal_world_bearer_token: SecretStr | None = None
    personal_world_workload_hmac_secret: SecretStr | None = None

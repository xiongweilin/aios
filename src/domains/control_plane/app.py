...
def _task_response(bridge: PersonalRuntimeBridge, state: Any) -> TaskResponse:
...
def create_app(
    ...
    *,
    ...
    providers.register(environment_provider)

    runtime_client = WorldRuntimeClient(
        cfg.world_runtime_base_url,
        timeout_seconds=cfg.world_runtime_timeout_seconds,
        transport=runtime_transport,
        bearer_token=cfg.world_runtime_bearer_token,
        delegation_id=cfg.world_runtime_delegation_id,
    )
    runtime_client.ensure_contracts()
    ...
    reconcile_repair_state(bridge, stale_after_seconds=900)
    profile_metrics = ControlPlaneMetricsCollector(bridge)
    ...
    app.state.domain_journal = journal
    ...
    async def notify(
        ...
        *,
        ...
        if not cfg.notification_enabled:
            return base
        try:
            effect_work_id = work_id
            if not effect_work_id:
                effect_work_id = bridge.ensure_auxiliary_work("notifications").id
            result = await bridge.invoke_capability(
                effect_work_id,
                "notify.send",
    ...
    async def runtime_health() -> dict[str, Any]:
        ...
        async with health_lock:
            ...
            if health_cache is not None and now - health_cache[0] < 5:
                return health_cache[1]
            try:
                local_provider_health = await asyncio.wait_for(providers.health(), timeout=5)
            except Exception as exc:
                local_provider_health = [
                    {
                        "provider_id": "control-plane-provider-health",
                        "available": False,
                        "detail": f"health probe failed: {type(exc).__name__}",
                    }
                ]
            value = {
                "runtime": runtime_client.health(),
                "providers": local_provider_health,
            }
    ...
    async def refresh_metrics_background() -> None:
        ...
        def collect_metrics() -> bytes:
            return b"".join((generate_latest(), generate_latest(profile_registry)))
    ...
    def append_alert_event(event_type: str, spec: AlertRepair, **extra: Any) -> None:
        ...
        journal.append(
            stream=f"alert:{spec.fingerprint}",
            kind=event_type,
    ...
    def escalation_already_sent(fingerprint: str) -> bool:
        events = journal.events(stream=f"alert:{fingerprint}")
        resolved = [event for event in events if event.kind == ALERT_RESOLVED_EVENT]
        last_resolved = resolved[-1] if resolved else None
        return any(
            event.kind == ALERT_ESCALATED_EVENT
            and (last_resolved is None or event.recorded_at > last_resolved.recorded_at)
            and (
    ...
    def recoverable_alert_specs() -> tuple[dict[str, str], list[AlertRepair]]:
        ...
        journal_events = [
            ...
            if event.kind in journal_types and event.stream.startswith("alert:")
        ]
        for event in journal_events:
            fingerprint = str(
                event.payload.get("fingerprint") or event.stream.removeprefix("alert:")
            )
            latest[fingerprint] = event
            if event.kind == ALERT_QUEUED_EVENT:
                ...
                queued[fingerprint] = AlertRepair(
                    ...
                    maintenance_parameters=(
                        ...
                        if isinstance(
                            parameters := event.payload.get("maintenance_parameters"),
                            dict,
                        )
                        else {}
                    ),
        ...
        for fingerprint, event in latest.items():
            if event.kind == ALERT_RESOLVED_EVENT:
                continue
            controller_id = str(event.payload.get("controller_id", ""))
            if controller_id:
            ...
            if event.kind == ALERT_QUEUED_EVENT:
                ...
                if spec is not None and controller_id and controller.get(controller_id) is not None:
                    pending.append(spec)
            elif event.kind == ALERT_FINISHED_EVENT and event.payload.get("status") in {
                "failed",
                "timeout",
            }:
                spec = queued.get(fingerprint)
                ...
                if current_fingerprints is None:
                    ...
                    for fingerprint, controller_id in recovered.items():
                        ...
                        await asyncio.to_thread(
                            journal.append,
                            stream=f"alert:{fingerprint}",
                            kind=ALERT_RESOLVED_EVENT,
                            event_id=new_id("event"),
                            payload={
                                "fingerprint": fingerprint,
    ...
    async def continue_waiting_controller(controller_id: str, command: str) -> TaskResponse:
        ...
        journal.append(
            ...
            kind=PERSONAL_HUMAN_INSTRUCTION_EVENT,
            event_id=new_id("event"),
    ...
    async def metrics() -> Response:
        ...
        content = (
            b"".join((generate_latest(), generate_latest(profile_registry)))
            if metrics_content_cache is None
            else metrics_content_cache[1]
        )
    ...
    async def runtime_view(x_control_plane_key: str = Header(default="")) -> dict[str, Any]:
        ...
        return {
            "runtime": await runtime_health(),
            "work": [item.model_dump(mode="json") for item in bridge.list_work()[-50:]],
            "capability_policies": [asdict(item) for item in CAPABILITY_POLICIES],
        }

...
def create_canary_proxy(
    ...
    *,
    ...
    async def proxy(request: Request, path: str) -> Response:
        ...
        upstream_path = request.scope.get("path", "/")
        ...
        if isinstance(root_path, str) and root_path not in {"", "/"}:
            ...
            if upstream_path == normalized_root:
                upstream_path = "/"
            elif upstream_path.startswith(normalized_root + "/"):
                upstream_path = upstream_path[len(normalized_root) :]
        target = base_url.rstrip("/") + upstream_path
